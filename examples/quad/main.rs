...
#[cfg(feature = "metal")]
extern crate gfx_backend_metal as GFXBackend;
#[cfg(feature = "gl")]
extern crate gfx_backend_gl as GFXBackend;
...
fn main() {
    ...
    #[cfg(feature = "metal")]
    let mut autorelease_pool = unsafe { GFXBackend::AutoreleasePool::new() };

    let mut events_loop = winit::EventsLoop::new();

    let _window = window::create_window(&events_loop);

    let window_size = _window.get_inner_size().unwrap();
    ...
    let (_instance, mut adapters, mut surface) = {
        let instance = GFXBackend::Instance::create("gfx-rs quad", 1);
    ...
    let (mut adapters, mut surface) = {
        let surface = GFXBackend::Surface::from_window(_window);
    ...
    let shader_lib = device.create_shader_library_from_source(
            ...
            GFXBackend::LanguageVersion::new(2, 0),
    ...
    */
    ...
    let pipelines = {
        let (vs_entry, fs_entry) = (
            pso::EntryPoint::<GFXBackend::Backend> {
                entry: ENTRY_NAME,
                module: &vs_module,
                specialization: &[
                    Specialization {
